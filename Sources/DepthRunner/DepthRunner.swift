...
struct DepthRunner {
    static func main() {
        do {
            ...
            var plausibilityWarnings = Set<PlausibilityWarning>()
            ...
            if options.requiresPointCloud {
                ...
                if samples.isEmpty {
                    ...
                    let roiPixels = roiInfo.sampleCount
                    let roiCoveragePercent: Double
                    if totalPixelCount > 0 {
                        roiCoveragePercent = Double(roiPixels) / Double(totalPixelCount) * 100
                    } else {
                    ...
                    if roiInfo.sampleCount == 0 {
                        ...
                        if filteredSamples.isEmpty {
                            ...
                            if options.volumeRequested {
                                ...
                                let preScaleStats = simdPoints.withUnsafeBufferPointer { buffer -> VolumeStats in
                                    computeAABBVolume(points: buffer, unit: .ml)
                                }
                                let pointCountBeforeVolume = simdPoints.count
                                ...
                                computeAndLogVolume(
                                    for: simdPoints,
                                    pointCountBeforeVolume: pointCountBeforeVolume,
                                    roiPixels: roiPixels,
                                    roiCoveragePercent: roiCoveragePercent,
                                    preScaleStats: preScaleStats,
            ...
            if options.strictWarn {
                let orderedWarnings = PlausibilityWarning.allCases.filter { plausibilityWarnings.contains($0) }
                if !orderedWarnings.isEmpty {
                    let summary = orderedWarnings.map { $0.rawValue }.joined(separator: ",")
                    print("WARN: \(summary)")
                }
            }
        } catch let error as DepthRunnerError {
...
private enum AutoScaleSkipReason {
...
private enum AutoScaleOutcome {
    ...
    case skipped(reason: AutoScaleSkipReason, measured: Float?)
    case disabled
}
...
private func applyAutoScaleIfNeeded(points: inout [SIMD3<Float>], assumedDiameterM: Float, disable: Bool) -> AutoScaleOutcome {
    ...
    guard points.count >= 500 else {
        return .skipped(reason: .insufficientPoints, measured: nil)
    }
    ...
    guard measuredDiameter.isFinite, measuredDiameter > 0 else {
        return .skipped(reason: .invalidMeasurement, measured: nil)
    }

    if measuredDiameter < kAutoScaleMinDiameterM || measuredDiameter > kAutoScaleMaxDiameterM {
        return .skipped(reason: .outOfRange, measured: measuredDiameter)
    }
...
private enum PlausibilityWarning: String, CaseIterable {
    case lowPoints = "low_points"
    case tinyROI = "tiny_roi"
    case oddZSpan = "odd_zspan"
    case volumeOutOfRange = "volume_out_of_range"
}

private func computeAndLogVolume(
    for points: [SIMD3<Float>],
    pointCountBeforeVolume: Int,
    roiPixels: Int,
    roiCoveragePercent: Double,
    preScaleStats: VolumeStats,
    unit: VolumeUnit,
    autoScaleOutcome: AutoScaleOutcome,
    warnings: inout Set<PlausibilityWarning>
) {
    let finalStats = points.withUnsafeBufferPointer { buffer -> VolumeStats in
        computeAABBVolume(points: buffer, unit: unit)
    }

    let coverageDisplay = max(0, min(100, roiCoveragePercent))
    let pointsLine = String(
        format: "Points: total=%d, used=%d, roi_pixels=%d, roi_cov=%.1f%%",
        pointCountBeforeVolume,
        finalStats.numPointsUsed,
        roiPixels,
        coverageDisplay
    )
    print(pointsLine)
    ...
    let bboxLine = String(
        format: "BBox[m]: pre x:[%.4f, %.4f] y:[%.4f, %.4f] z:[%.4f, %.4f] | post x:[%.4f, %.4f] y:[%.4f, %.4f] z:[%.4f, %.4f]",
        Double(preScaleStats.bboxMin.x),
        Double(preScaleStats.bboxMax.x),
        Double(preScaleStats.bboxMin.y),
        Double(preScaleStats.bboxMax.y),
        Double(preScaleStats.bboxMin.z),
        Double(preScaleStats.bboxMax.z),
        Double(finalStats.bboxMin.x),
        Double(finalStats.bboxMax.x),
        Double(finalStats.bboxMin.y),
        Double(finalStats.bboxMax.y),
        Double(finalStats.bboxMin.z),
        Double(finalStats.bboxMax.z)
    )
    print(bboxLine)

    let assumedDisplay = String(format: "%.2f", Double(kAssumedPlateDiameterM))
    let scaleLine: String
    switch autoScaleOutcome {
    case .applied(let measured, let factor):
        scaleLine = String(
            format: "Scale: assumed=%@ m, measured=%.4f m, factor=%.3f",
            assumedDisplay,
            Double(measured),
            Double(factor)
        )
    case .skipped(let reason, let measured):
        let measuredDisplay = measured.map { String(format: "%.4f", Double($0)) } ?? "n/a"
        let reasonText: String
        switch reason {
        case .insufficientPoints:
            reasonText = "insufficient_points"
        case .invalidMeasurement:
            reasonText = "invalid_measurement"
        case .outOfRange:
            reasonText = "out_of_range"
        }
        scaleLine = "Scale: assumed=\(assumedDisplay) m, skipped(reason=\(reasonText), measured=\(measuredDisplay))"
    case .disabled:
        scaleLine = "Scale: assumed=\(assumedDisplay) m, disabled via --no-auto-scale"
    }
    print(scaleLine)

    let dx = Double(finalStats.bboxMax.x - finalStats.bboxMin.x)
    let dy = Double(finalStats.bboxMax.y - finalStats.bboxMin.y)
    let dz = Double(finalStats.bboxMax.z - finalStats.bboxMin.z)
    let volumeM3 = max(0, dx * dy * dz)
    let volumeML = volumeM3 * 1_000_000

    var volumeLine = String(format: "Volume: %.1f ml", volumeML)
    switch unit {
    case .ml:
        break
    case .cm3:
        volumeLine.append(String(format: " (%.1f cm3)", volumeML))
    case .m3:
        volumeLine.append(String(format: " (%.6f m3)", volumeM3))
    }
    print(volumeLine)

    if finalStats.numPointsUsed < 1000 {
        print("⚠️ Low point count (used=\(finalStats.numPointsUsed), total=\(pointCountBeforeVolume)) – result may be unstable")
        warnings.insert(.lowPoints)
    }

    if coverageDisplay < 10 {
        print(String(format: "⚠️ Tiny ROI coverage (%.1f%%)", coverageDisplay))
        warnings.insert(.tinyROI)
    }

    let zSpan = dz
    if zSpan < 0.01 || zSpan > 0.20 {
        print(String(format: "⚠️ Suspicious Z-span (%.4f m)", zSpan))
        warnings.insert(.oddZSpan)
    }

    if volumeML < 10 || volumeML > 10_000 {
        print(String(format: "⚠️ Volume out of nominal range (%.1f ml)", volumeML))
        warnings.insert(.volumeOutOfRange)
    }
}
